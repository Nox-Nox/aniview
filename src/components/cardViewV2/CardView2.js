...
<ThemeProvider theme={responsiveFontSizes(mainTheme)}>
            {(props.items || []).map((item) =>(
                ...
                <div id={styles.totalep}>{item.episodes}</div>
                <div id={styles.duration}>{item.duration}</div>
                <div id={styles.timer}></div>
                <div id={styles.description} dangerouslySetInnerHTML={{ __html: item.description }} onMouseLeave={() => {
